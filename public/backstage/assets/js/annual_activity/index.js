...
var myData = database.ref('/annual_activity');
...
myData.on('value', function(snapshot) {
  ...
  let displayRows = ` <thead> <tr>
          ...
          <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2 align-middle">Content Title</th>
          <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2 align-middle">Body Text</th>
          <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 align-middle">External Link</th>
  ...
  for(let i of ActivityListKey){
    ...
    <tr>
      <td>
        ...
        <td class="col text-center justify-content-md-center">
        ...
        <td class="col text-center justify-content-md-center">
          <a href="${activityProfile[i].link}" class="btn btn-outline-info " role="button" aria-pressed="true" target="_blank">View Link</a>
        </td>
...
const newMember = async(id)=> {
  document.getElementById('modal-title-default').innerText ="新增Annual Events"
...
const read = async(id)=> {
  ...
  document.getElementById('modal-title-default').innerText ="修改Annual Events"

  try{
    const usersRef = database.ref(`/annual_activity/${id}`);
...
const deleteRow = async(id)=> {
  swal({
    ...
    text: "Once deleted, this partner information cannot be restored.",
    icon: "warning",
  ...
  .then(async (willDelete) => {
    if (willDelete) {
      ...
      swal("the partner information has been deleted.", {
        icon: "success",
      });
      database.ref(`/annual_activity/${id}`).remove();
    } else {
      // swal("沒問題，沒有進行任何操作");
    }
  });
...
const create = async()=> {
  ...
  try{
    ...
    firebase.database().ref('/annual_activity/' + Object.keys(memberCount).length).set(inputDict);
...
const upload = async(id)=> {
  ...
  try{
    await database.ref(`/annual_activity/${id}`).update(inputDict);
    document.getElementById('update-content').classList.remove("btn-dark")
    document.getElementById('update-content').classList.add("btn-success")
    document.getElementById('update-content').innerHTML = `Success`
    await sleep(3000);
