...
const read = async(id)=> {
  ...
  try{
    const usersRef = database.ref(`/main/annual_activity/${id}`);
...
const create = async()=> {
  ...
  try{
    ...
    firebase.database().ref('/main/annual_activity/' + Object.keys(memberCount).length).set(inputDict);
...
const upload = async(id)=> {
  ...
  try{
    await database.ref(`/main/annual_activity/${id}`).update(inputDict);
    document.getElementById('update-content').classList.remove("btn-dark")
    document.getElementById('update-content').classList.add("btn-success")
    document.getElementById('update-content').innerHTML = `修改成功`
    await sleep(3000);
