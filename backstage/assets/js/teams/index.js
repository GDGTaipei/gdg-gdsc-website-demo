...
myData.on('value', function(snapshot) {
  ...
  let displayRows = ` <thead> <tr>
          ...
          <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2 align-middle">姓</th>
          <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 align-middle">名</th>
          <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 align-middle">頭銜</th>
...
const deleteRow = async(id)=> {
  ...
  .then(async (willDelete) => {
    if (willDelete) {
      ...
      database.ref(`/taipei/team/${id}`).remove();
    } else {
      // swal("沒問題，沒有進行任何操作");
    }
  });
